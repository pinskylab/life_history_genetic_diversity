...
############### mtDNA: Multiple Analyses ############### 

#Max Length vs He (w/ Fecundity Mean)
(split_plot <- ggplot(aes(maxlength, He), data = mtdna_data) + 
   geom_point() + 
   facet_wrap(~ fecundity_mean) + # create a facet for each mountain range
   xlab("Max Length") + 
   ylab("He"))
...
#Max Length vs He (w/ Fecundity Mean)
(split_plot <- ggplot(aes(fecundity_mean, He), data = msat_data, msat_data[!is.na(msat_data$Condition),]) + 
   geom_point() + 
   facet_wrap(~ maxlength) + # create a facet for each mountain range
   xlab("Fecundity Mean") + 
   ylab("He"))
...
isolatedmtdna_data <- mtdna_data %>% 
    filter(spp %in% c("Aetobatus narinari", "Anoplopoma fimbria", "Clupea pallasii pallasii", "Cynoscion nebulosus","Engraulis eurystole", "Epinephelus striatus", "Gadus macrocephalus",
                      "Ginglymostoma cirratum", "Lutjanus analis","Merluccius bilinearis", "Pagrus pagrus", "Sebastes flavidus", "Syngnathus leptorhynchus"))

isolatedmsat_data <- msat_data %>%
  filter(spp %in% c("Aetobatus narinari", "Anoplopoma fimbria", "Clupea pallasii pallasii", "Cynoscion nebulosus","Engraulis eurystole", "Epinephelus striatus", "Gadus macrocephalus",
                    "Ginglymostoma cirratum", "Lutjanus analis","Merluccius bilinearis", "Pagrus pagrus", "Sebastes flavidus", "Syngnathus leptorhynchus"))

######
isolatedmtdna_data$final_fertilization <- NA #create new column to categorize fertilization methods

isolatedmtdna_data$final_fertilization [isolatedmtdna_data$fertilization =="external"]  <- "external"
isolatedmtdna_data$final_fertilization [isolatedmtdna_data$fertilization =="internal (oviduct)"] <- "internal (oviduct)"
isolatedmtdna_data$final_fertilization [isolatedmtdna_data$fertilization =="in brood pouch or similar structure"] <- "internal (oviduct)" #convert "in brood pouch or similar structure" to internal fertilization

id_mtdna_final_fertilization_He_no.na <- isolatedmtdna_data[!is.na(isolatedmtdna_data$final_fertilization) & !is.na(isolatedmtdna_data$He),] #create new table that excludes NA's from columns of interest

theme_update(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) #centered plot title

ggplot(id_mtdna_final_fertilization_He_no.na) + geom_boxplot(aes(x = final_fertilization, y = He)) + #final fertilization & He box plot
  ggtitle("mtDNA: Fertilization Method vs. He") + #add plot title
  xlab("Fertilization Method") + ylab("He") + #add axis labels
  theme(                                 #specify characteristics of the plot 
    plot.title = element_text(size=14, face="bold"), 
...
isolatedmtdna_data$final_reproductionmode <- NA #create new column to categorize reproduction mode

isolatedmtdna_data$final_reproductionmode  [isolatedmtdna_data$reproductionmode =="dioecism"]  <- "Dioecious"
isolatedmtdna_data$final_reproductionmode  [isolatedmtdna_data$reproductionmode =="protogyny"] <- "Hermaphrodite" #for protogyny, label as "Hermpahrodites"
isolatedmtdna_data$final_reproductionmode  [isolatedmtdna_data$reproductionmode =="protandry"] <- "Hermaphrodite" #for protandry, label as "Hermpahrodites"
isolatedmtdna_data$final_reproductionmode  [isolatedmtdna_data$reproductionmode =="true hermaphroditism"] <- "Hermaphrodite" #for true hermaphroditism, label as "Hermpahrodites"

id_mtdna_final_reproductionmode_He_no.na <- isolatedmtdna_data[!is.na(isolatedmtdna_data$final_reproductionmode ) & !is.na(isolatedmtdna_data$He),] #create new table that excludes NA's from columns of interest

ggplot(id_mtdna_final_reproductionmode_He_no.na ) + geom_boxplot(aes(x = final_reproductionmode , y = He)) + #final reproduction mode & He box plot
  ggtitle("mtDNA: Reproduction Mode vs. He") + #add plot title
  xlab("Reproduction Mode ") + ylab("He") + #add axis labels
  theme(                                 #specify characteristics of the plot 
    plot.title = element_text(size=14, face="bold"), 
...
isolatedmtdna_data$specific.repro_mode <- NA #create new column to categorize hermaphrodite type

isolatedmtdna_data$specific.repro_mode  [isolatedmtdna_data$reproductionmode =="dioecism"]  <- "Dioecism"
isolatedmtdna_data$specific.repro_mode  [isolatedmtdna_data$reproductionmode =="protogyny"] <- "Protogyny"
isolatedmtdna_data$specific.repro_mode  [isolatedmtdna_data$reproductionmode =="protandry"] <- "Protandry"
isolatedmtdna_data$specific.repro_mode  [isolatedmtdna_data$reproductionmode =="true hermaphroditism"] <- "True Hermaphroditism"

id_mtdna_reproduction_type_He_no.na <- isolatedmtdna_data[!is.na(isolatedmtdna_data$specific.repro_mode ) & !is.na(isolatedmtdna_data$He),] #create new table that excludes NA's from columns of interest

ggplot(id_mtdna_reproduction_type_He_no.na ) + geom_boxplot(aes(x = specific.repro_mode , y = He)) + #hermaphrodite type mode & He box plot
  ggtitle("mtDNA: Specific Reproduction Modes vs. He") + #add plot title
  xlab("Specific Reproduction Mode") + ylab("He") + #add axis labels
  theme(                                 #specify characteristics of the plot 
    plot.title = element_text(size=14, face="bold"), 
...
id_mtdna_maxlength_He_no.na <- isolatedmtdna_data[!is.na(isolatedmtdna_data$maxlength) & !is.na(isolatedmtdna_data$He),] #create new table that excludes NA's from columns of interest

id_mtdna_maxlength_He_no.na$logtransform.maxlength <- NA #add column to do a log transformation for max length

for (i in 1:nrow(id_mtdna_maxlength_He_no.na)) { #get log transformation data
  cat(paste(i, " ", sep = ''))
  id_mtdna_maxlength_He_no.na$logtransform.maxlength <- log10(id_mtdna_maxlength_He_no.na$maxlength)
}
...
ggplot(id_mtdna_maxlength_He_no.na, aes(x=logtransform.maxlength, y = He)) + #max length & He scatter plot
  geom_point(shape=1) +    # Use hollow circles
  geom_smooth(method=lm,   # Add linear regression line
              se=TRUE, formula = y ~ x) +
  ylim(0,1)+                              #create limits
  ...
  theme(                                 #specify characteristics of the plot 
    ...
    axis.title.x = element_text(color="blue", size=14, face="bold"),
    axis.title.y = element_text(color="red", size=14, face="bold"))#+
  #annotate(geom="label", x = 2.4, y = .807, label = lm_eqn(id_mtdna_maxlength_He_no.na$logtransform.maxlength, id_mtdna_maxlength_He_no.na$He, mtdna_maxlength_He_no.na), 
   #        color="black", size = 5, parse=TRUE, alpha=0.80)  #add regression line equation

#Fecundity Mean##
id_mtdna_fecundity_He_no.na <- isolatedmtdna_data[!is.na(isolatedmtdna_data$fecundity_mean) & !is.na(isolatedmtdna_data$He),] #create new table that excludes NA's from columns of interest

id_mtdna_fecundity_He_no.na$logtransform.fecundity <- NA #add column to do a log transformation for fecundity mean

for (i in 1:nrow(id_mtdna_fecundity_He_no.na)) { #get log transformation data
  cat(paste(i, " ", sep = ''))
  id_mtdna_fecundity_He_no.na$logtransform.fecundity <- log10(id_mtdna_fecundity_He_no.na$fecundity_mean)
}

ggplot(id_mtdna_fecundity_He_no.na, aes(x= logtransform.fecundity, y= He)) + #fecundity mean & He scatter plot
  geom_point(shape=1) +    # Use hollow circles
  geom_smooth(method=lm,   # Add linear regression line
              se=TRUE) +
  ylim(0,1)+                              #create limits
...
#Max Length#
plot(density(id_mtdna_maxlength_He_no.na$maxlength), main="Max Length Density") #create density plot for max length
polygon(density(id_mtdna_maxlength_He_no.na$maxlength), main="Max Length Density", col="blue") #specify characteristics of plot

#Fecundity Mean#
plot(density(id_mtdna_fecundity_He_no.na$fecundity_mean), main="Fecundity Mean Density") #create density plot for fecundity mean
polygon(density(id_mtdna_fecundity_He_no.na$fecundity_mean), main="Fecundity Mean Density", col="blue") #specify characteristics of plot
...
#Fertilization#
isolatedmsat_data$final_fertilization <- NA #create new column to categorize fertilization methods

isolatedmsat_data$final_fertilization [isolatedmsat_data$fertilization =="external"]  <- "external"
isolatedmsat_data$final_fertilization [isolatedmsat_data$fertilization =="internal (oviduct)"] <- "internal (oviduct)"
isolatedmsat_data$final_fertilization [isolatedmsat_data$fertilization =="in brood pouch or similar structure"] <- "internal (oviduct)" #convert "in brood pouch or similar structure" to internal fertilization

id_msat_final_fertilization_He_no.na <- isolatedmsat_data[!is.na(isolatedmsat_data$final_fertilization) & !is.na(isolatedmsat_data$He),] #create new table that excludes NA's from columns of interest

ggplot(id_msat_final_fertilization_He_no.na) + geom_boxplot(aes(x = final_fertilization, y = He)) + #final fertilization & He box plot
  ggtitle("msat: Fertilization Method vs. He") + #add plot title
  xlab("Fertilization Method") + ylab("He") + #add axis labels
  theme(                                 #specifying characteristics of the plot 
    plot.title = element_text(size=14, face="bold"),
...
#Reproduction Mode#
isolatedmsat_data$final_reproductionmode <- NA #create new column to categorize reproduction mode

isolatedmsat_data$final_reproductionmode  [isolatedmsat_data$reproductionmode =="dioecism"]  <- "Dioecious"
isolatedmsat_data$final_reproductionmode  [isolatedmsat_data$reproductionmode =="protogyny"] <- "Hermaphrodite" #for protogyny, label as "Hermpahrodites"
isolatedmsat_data$final_reproductionmode  [isolatedmsat_data$reproductionmode =="protandry"] <- "Hermaphrodite" #for protandry, label as "Hermpahrodites"
isolatedmsat_data$final_reproductionmode  [isolatedmsat_data$reproductionmode =="true hermaphroditism"] <- "Hermaphrodite" #for true hermaphroditism, label as "Hermpahrodites"

id_msat_final_reproductionmode_He_no.na <- isolatedmsat_data[!is.na(isolatedmsat_data$final_reproductionmode ) & !is.na(isolatedmsat_data$He),] #create new table that excludes NA's from columns of interest

ggplot(id_msat_final_reproductionmode_He_no.na ) + geom_boxplot(aes(x = final_reproductionmode , y = He)) + #final reproduction mode & He box plot
  ggtitle("msat: Reproduction Mode vs. He") + #add plot title
  xlab("Reproduction Mode ") + ylab("He") + #add axis labels
  theme(                                 #specifying characteristics of the plot 
    plot.title = element_text(size=14, face="bold"),
...
isolatedmsat_data$specific.repro_mode <- NA #create new column to categorize reproduction type

isolatedmsat_data$specific.repro_mode  [isolatedmsat_data$reproductionmode =="dioecism"]  <- "Dioecism"
isolatedmsat_data$specific.repro_mode  [isolatedmsat_data$reproductionmode =="protogyny"] <- "Protogyny"
isolatedmsat_data$specific.repro_mode  [isolatedmsat_data$reproductionmode =="protandry"] <- "Protandry"
isolatedmsat_data$specific.repro_mode  [isolatedmsat_data$reproductionmode =="true hermaphroditism"] <- "True Hermaphroditism"

id_msat_reproduction_type_He_no.na <- isolatedmsat_data[!is.na(isolatedmsat_data$specific.repro_mode ) & !is.na(isolatedmsat_data$He),] #create new table that excludes NA's from columns of interest

ggplot(id_msat_reproduction_type_He_no.na ) + geom_boxplot(aes(x = specific.repro_mode , y = He)) + #hermaphrodite type mode & He box plot
  ggtitle("msat: Specific Reproduction Modes vs. He") + #add plot title
  xlab("Specific Reproduction Mode") + ylab("He") + #add axis labels
  theme(                                 #specify characteristics of the plot 
    plot.title = element_text(size=14, face="bold"), 
...
#w/ Rhincodon typus outlier
id_msat_maxlength_He_no.na <- isolatedmsat_data[!is.na(isolatedmsat_data$maxlength) & !is.na(isolatedmsat_data$He),] #create new table that excludes NA's from columns of interest

id_msat_maxlength_He_no.na$logtransform.maxlength <- NA #add column to do a log transformation for max length

for (i in 1:nrow(id_msat_maxlength_He_no.na)) { #get log transformation data
  cat(paste(i, " ", sep = ''))
  id_msat_maxlength_He_no.na$logtransform.maxlength <- log10(id_msat_maxlength_He_no.na$maxlength)
}

ggplot(id_msat_maxlength_He_no.na, aes(x= logtransform.maxlength, y= He)) + #max length & He scatter plot
  geom_point(shape=1) +    # Use hollow circles
  geom_smooth(method=lm,   # Add linear regression line
              se=TRUE) + 
  ylim(0,1)+                              #create limits
...
#Fecundity Mean#
id_msat_fecundity_He_no.na <- isolatedmsat_data[!is.na(isolatedmsat_data$fecundity_mean) & !is.na(isolatedmsat_data$He),] #create new table that excludes NA's from columns of interest

id_msat_fecundity_He_no.na$logtransform.fecundity <- NA #add column to do a log transformation for fecundity mean

for (i in 1:nrow(id_msat_fecundity_He_no.na)) { #get log transformation data
  cat(paste(i, " ", sep = ''))
  id_msat_fecundity_He_no.na$logtransform.fecundity <- log10(id_msat_fecundity_He_no.na$fecundity_mean)
}

ggplot(id_msat_fecundity_He_no.na, aes(x= logtransform.fecundity, y= He)) + #fecundity mean & He scatter plot
  geom_point(shape=1) +    # Use hollow circles
  geom_smooth(method=lm,   # Add linear regression line
              se=TRUE) +
  ylim(0,1)+                              #create limits
...
#Max Length#
plot(density(id_msat_maxlength_He_no.na$maxlength), main="msat: Max Length Density") #create density plot for max length
polygon(density(id_msat_maxlength_He_no.na$maxlength), main="msat: Max Length Density", col="blue") #specify characteristics of plot

#Fecundity Mean#
plot(density(id_msat_fecundity_He_no.na$fecundity_mean), main="msat: Fecundity Mean Density") #create density plot for fecundity mean
polygon(density(id_msat_fecundity_He_no.na$fecundity_mean), main="msat: Fecundity Mean Density", col="blue") #specify characteristics of plot
